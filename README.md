...
The standard leftover scan checks exact product-named folders in AppData and ProgramData after an uninstall is confirmed. Manual delete lists the registered install and uninstaller folders, exact app-data matches in AppData and ProgramData, and exact-name matches throughout accessible subfolders in common personal libraries. It never selects a result automatically, and a matching name is not proof that a folder belongs to the app. Review each path carefully; deletion is permanent and may remove program or personal files. It does not scan the whole disk or identify individual files by content.
